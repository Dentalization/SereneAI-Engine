...
def render_chat_interface(config):
            ...
            if "image_path" in message and message["image_path"]:
                if os.path.exists(message["image_path"]):
                    try:
                        ...
                        logging.debug(f"UI: Displayed historical image: {message['image_path']}")
                    except Exception as e:
                        st.error(f"Could not display image: {str(e)}")
                        logging.error(f"UI: Image display error: {str(e)}")
                else:
                    st.warning("Image file no longer available")
                    logging.warning(f"UI: Missing historical image: {message['image_path']}")
    ...
    if prompt:
        ...
        logging.info(f"UI: User message appended to history (len now: {len(st.session_state.messages)})")
        ...
        with st.chat_message("assistant"):
            ...
            try:
                with st.spinner("Processing..."):
                    ...
                    full_result = run_agent(user_text, image_path,
                                            st.session_state.messages[:-1])  # Dict: {'response', 'sources'}
                ...
                logging.info(f"UI: Response streamed (length: {len(response)} chars, sources: {len(sources)})")
                        ...
                        for src in sources:
                            ...
                            with col1:
                                if 'PubMed' in src['source']:
                                    st.markdown(f"**Source {src['id']} (PubMed):** {src['title']}")
                                    if src['authors']:
                                    ...
                                    st.markdown(f"**Source {src['id']} (PDF):** {src['title']} (Page {src['page']})")
                            with col2:
                                if 'PubMed' in src['source']:
                                    st.image("https://cdn.ncbi.nlm.nih.gov/coreutils/nwds/img/favicons/favicon.png",
                                             width=50)  # PubMed icon
                            st.caption(src['snippet'])
                            st.markdown("---")
                        logging.debug(f"UI: Displayed {len(sources)} sources in expander")
            except Exception as e:
        ...
        st.session_state.messages.append(
            {"role": "assistant", "content": response, "sources": sources})  # Store sources in message
        logging.debug(f"UI: Assistant message appended (snippet: {response[:100]}...)")

        db_input = user_text if user_text else "(Image query)"
        try:
            c = conn.cursor()
            c.execute("INSERT INTO history (user_input, response) VALUES (?, ?)", (db_input, response))
            conn.commit()
            ...
            try:
                ...
                if annotated_path and os.path.exists(annotated_path):
                    with st.chat_message("assistant"):
                        ...
                        logging.info(f"UI: Displayed annotated image & insights (snippet: {spatial_insights[:100]}...)")
                    st.session_state.messages.append({
                        "role": "assistant",
